...
var _ = Describe("ResultTest", func() {
	var (
	...
	Describe("RegisterUser", func() {
		It("should create RegisterUser response", func(done Done) {
			...
			mockCmd := createCmd(producer.Input(), "RegisterUser", reqTopic, respTopic, marshalUser)
			...
			msgCallback := func(msg *sarama.ConsumerMessage) bool {
				...
				if doc.CorrelationID == mockCmd.UUID {
					...
					userModel := &user.User{}
					err = json.Unmarshal(doc.Data, userModel)
					Expect(err).ToNot(HaveOccurred())
	...
	Describe("DeleteUser", func() {
		It("should create DeleteUser response", func(done Done) {
			...
			mockCmd := createCmd(producer.Input(), "DeleteUser", reqTopic, respTopic, marshalUser)
			...
			msgCallback := func(msg *sarama.ConsumerMessage) bool {
				...
				if doc.CorrelationID == mockCmd.UUID {
					...
					delResult := map[string]interface{}{}
					err = json.Unmarshal(doc.Data, &delResult)
					Expect(err).ToNot(HaveOccurred())
	...
	Describe("UpdateUser", func() {
		It("should create UpdateUser response", func(done Done) {
			...
			mockCmd := createCmd(producer.Input(), "UpdateUser", reqTopic, respTopic, marshalParams)
			...
			msgCallback := func(msg *sarama.ConsumerMessage) bool {
				...
				if doc.CorrelationID == mockCmd.UUID {
					...
					err = json.Unmarshal(doc.Data, &updateResult)
					Expect(err).ToNot(HaveOccurred())
